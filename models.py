...
import logging
from modules import DecoderFC,DecoderCONV,DecoderRGB
...
class ProtoGMVAE(nn.Module):
    ...
    def __init__(self, k, num_classes, Qy_x_net, Qz_xy_net, Px_z_net):
        super(ProtoGMVAE, self).__init__()
        self.k = k
        self.qy_x = Qy_x_net
        self.qz_xy = Qz_xy_net
        self.px_z = Px_z_net
        ...
        #Testing affecting specific class to prototypes
        #self.prototype_class_identity = torch.zeros(self.k, self.num_classes)

        #self.num_prototypes_per_class = self.k // self.num_classes
        #for j in range(self.k):
        #    self.prototype_class_identity[j, j // self.num_prototypes_per_class] = 1
        
        self.classif_layer = nn.Sequential(nn.Linear(self.k, self.num_classes, bias=False))
        #self.classif_layer = nn.Sequential(nn.Linear(self.k, self.k),
            #nn.ReLU(),
            #nn.Linear(self.k,self.num_classes),
            #)
        
        #self.set_last_layer_incorrect_connection(0)
        #nn.init.constant_(self.classif_layer[0].weight,0)
    ...
    def infer(self, x):
        ...
        z_hat, *_ = self.qz_xy(x, qy)
        *_, x_hat = self.px_z(z_hat, qy)
        pred_class = self.classif_layer(qy)
        out_infer = {
            "y": qy,
            "z": z_hat,
            "x_hat": x_hat,
    ...
    def forward(self, x):
        ...
        pred_class = self.classif_layer(qy)
        for i in range(k):
            print(f"cuda memory used: {torch.cuda.memory_allocated()/1024/1024}MB")
            y = y_ + torch.eye(k).to(x.device)[i]
            z[i], zm[i], zv[i] = self.qz_xy(x, y)
            #z[i], zm[i], zv[i] = self.qz_xy(x, qy)
            zm_prior[i], zv_prior[i], px[i] = self.px_z(z[i], y)
            #zm_prior[i], zv_prior[i], px[i] = self.px_z(z[i], qy)

        #_, z_mean, *_ = self.qz_xy(x, qy)
        #distances = torch.cdist(z_mean, zm_prior, p=2)
        #pred_class = self.classif_layer(z_mean)


        # Inference for x_hat:
        with torch.no_grad():
            #y_hat = torch.argmax(qy, dim=-1)
            #y_hat = F.softmax(qy, dim=-1)
            y_hat = qy
            #y_temp = torch.zeros(batch_size, k)
            #y_temp = torch.scatter(y_, 1, y_hat.unsqueeze(1), 1)
            z_hat, *_ = self.qz_xy(x, qy)
    ...
    def _visualize(self,y):
        """
        Create image from given mixture y using px_z decoder

        :param y: k-dimensional vector
        :return: a decoded image of the z variable obtained from pz_y with the input y
        """

        z_m = self.px_z.z_mean(y)
        decoded_x = self.px_z.decoder(z_m.unsqueeze(0))
        x_img = decoded_x.squeeze().detach().cpu().numpy()
        if isinstance(self.px_z.decoder,DecoderFC):
            x_img = np.reshape(x_img, (self.px_z.decoder.input_size,self.px_z.decoder.input_size))
        if self.px_z.decoder.output_channels == 3:
    ...
    def visualize_all_components(self):
        """
        Create image for each of the components of the mixture
        :return:
        """
        images = []

        for i in range(self.k):
            y = torch.zeros(self.k).to(self.device)
            y[i] = 1
            images.append(self._visualize(y))
    ...
    def set_last_layer_incorrect_connection(self, incorrect_strength):
        ...
        self.classif_layer[0].weight.data.copy_(
